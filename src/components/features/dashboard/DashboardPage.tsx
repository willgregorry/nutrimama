...
export default function DashboardPage() {
    ...
    return (
        <div className="flex flex-col gap-12 w-full pb-16 lg:pb-10">

            <section className="flex flex-col w-full min-w-0">
                <SectionHeader title="Kitab Ibu" href="/dashboard/kitab-ibu" />
                <div className="flex gap-4 sm:gap-6 w-full overflow-x-auto pb-4 scrollbar-hide snap-x">
                    {isLoading ? (
                        <div className="text-neutral-500 py-4" style={{ fontSize: "clamp(0.8rem, 1.2vw, 0.9rem)" }}>
                            Memuat artikel...
                        </div>
                    ) : (
                        articles?.slice(0, 5).map((article) => (
                            <Link
                                key={article.id}
                                href={`/dashboard/kitab-ibu/${article.kategori}/${article.id}`}
