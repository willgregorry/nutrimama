...
const labelMap: Record<string, string> = {
  proteinHewani: "Protein Hewani Utama (Ikan/Daging/Telur)",
  airPutih: "Air Putih (Minimal 10 gelas)",
  tabletTambahDarah: "Tablet Tambah Darah (TTD)",
  asamFolat: "Asam Folat / Vitamin D3",
  mualLemas: "Merasa Mual atau Lemas",
}

export default function LaporanHarianMasaKehamilanPage() {
  const router = useRouter()
  const data = useJurnalStore((state) => state.harianMasaKehamilan)

  useEffect(() => {
    if (!data) {
      router.replace("/dashboard/jurnal/masa-kehamilan/harian")
    }
  }, [data, router])
  ...
  return (
    <div className="flex w-full h-full justify-center lg:py-8">
      <div className="flex flex-col w-full max-w-2xl px-4 lg:px-0">
        ...
        <Link
          href="/dashboard/jurnal/masa-kehamilan/harian"
