...
const labelMap: Record<string, string> = {
  ...
  aktivitasFisik: "Aktivitas Fisik Ringan",
}

export default function LaporanMingguanMasaKehamilanPage() {
  const router = useRouter()
  const data = useJurnalStore((state) => state.mingguanMasaKehamilan)

  useEffect(() => {
    if (!data) {
      router.replace("/dashboard/jurnal/masa-kehamilan/mingguan")
    }
  }, [data, router])
  ...
  return (
    <div className="flex w-full h-full justify-center lg:py-8">
      <div className="flex flex-col w-full max-w-2xl px-4 lg:px-0">
        ...
        <Link
          href="/dashboard/jurnal/masa-kehamilan/mingguan"
