...
const labelMap: Record<string, string> = {
  vitaminA: "Vitamin A",
  beratBadanAnak: "Penimbangan Berat Badan Anak",
  tinggiAnak: "Pengukuran Tinggi/Panjang Anak",
  lingkarKepala: "Pengukuran Lingkar Kepala",
  imunisasiDasar: "Imunisasi Dasar",
  cekHemoglobin: "Cek Kadar Hemoglobin (Hb)",
}

export default function LaporanBulananPascaKelahiranPage() {
  const router = useRouter()
  const data = useJurnalStore((state) => state.bulananPascaKelahiran)

  useEffect(() => {
    if (!data) {
      router.replace("/dashboard/jurnal/pasca-kelahiran/bulanan")
    }
  }, [data, router])
  ...
  return (
    <div className="flex w-full h-full justify-center lg:py-8">
      <div className="flex flex-col w-full max-w-2xl px-4 lg:px-0">
        ...
        <Link
          href="/dashboard/jurnal/pasca-kelahiran/bulanan"
