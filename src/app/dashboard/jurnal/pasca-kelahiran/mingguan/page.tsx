...
const mingguanSchema = z.object({
  omega3: z.enum(["Terpenuhi", "Tidak Terpenuhi"], { message: "Pilih salah satu opsi" }),
})

type MingguanFormValues = z.infer<typeof mingguanSchema>

export default function MingguanPascaKelahiranPage() {
  const setMingguanPascaKelahiran = useJurnalStore((state) => state.setMingguanPascaKelahiran)
  ...
  const onSubmit = async (data: MingguanFormValues) => {
    ...
    setMingguanPascaKelahiran(data)
    router.push("/dashboard/jurnal/pasca-kelahiran/mingguan/laporan")
  }
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-8 w-full mb-12">
          <div className="flex flex-col gap-4">
            <label className="text-lg font-medium text-neutral-900 leading-snug">
              Apakah sudah ada mengkonsumsi Omega-3/Ikan laut?
            </label>
